	...
	def get_additional_columns(self):
		...
		if self.filters.party_type == "Customer":
			...
			result = (
				...
				.join(customer_credit_limit).on(customer.name == customer_credit_limit.parent)
				.select(
				...
				(customer_credit_limit.company == self.filters.get("company"))
				.run(as_dict=True)
			)
	...
	def get_gl_entries(self):
		...
		self.gl_entries = frappe.db.sql(
			...
			where
				...
				and gle.branch = %(branch)s and gle.posting_date <= %(to_date)s {conditions} 
		...
		gl_entries = frappe.db.sql(
			...
			where
				...
				and (voucher_type, voucher_no) in (
					...
					and gle.branch = %(branch)s and gle.posting_date between %(from_date)s and %(to_date)s and gle.docstatus < 2
				) and (voucher_type, voucher_no) in (
					select voucher_type, voucher_no from `tabGL Entry` gle
					where gle.party_type=%(party_type)s and ifnull(party, '') != ''
					and gle.branch = %(branch)s and gle.posting_date between %(from_date)s and %(to_date)s and gle.docstatus < 2 {conditions}
				)
		""".format(
